...
const CONFIG_DIR =
  `${process.env.XDG_CONFIG_HOME ?? `${process.env.HOME ?? ""}/.config`}/herdr-palette`
...
/**
 * All themes available to the switcher: bundled + ~/.config/herdr-palette/themes/*.json.
...
/**
 * Read ~/.config/herdr-palette/theme.json. Returns either:
...
/**
 * Combine the palette's declared theme with the user's ~/.config/herdr-palette/theme.json.
